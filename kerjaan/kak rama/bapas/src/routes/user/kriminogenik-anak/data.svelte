<script>
  ...
  async function dapatkanData() {
    if (browser) {
      ...
      for (let n in data) {
        // data[n].data.lahir = ubahTanggal(data[n].data.lahir);
...
<svelte:head>
  <title>Data Kriminogenik Anak</title>
  <Meta
    title="Data Kriminogenik Anak"
    description="Berisi dengan data kriminogenik anak yang siap diprint"
  />
</svelte:head>
